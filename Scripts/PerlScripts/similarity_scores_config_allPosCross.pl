...
unless (defined $infile and defined $outfile) {
    ...
    print STDERR "Usage: similarity_scores_config.pl inputfile outputfile\n";
    exit 1;
}
...
my $jcn = WordNet::Similarity::jcn->new($wn, "../PerScripts/config-files/config-jcn.conf");
...
my $res = WordNet::Similarity::res->new($wn, "../PerScripts/config-files/config-res.conf");
...
my $lin = WordNet::Similarity::lin->new($wn, "../PerScripts/config-files/config-lin.conf");
...
my $wup = WordNet::Similarity::wup->new($wn, "../PerScripts/config-files/config-wup.conf");
...
my $lch = WordNet::Similarity::lch->new($wn, "../PerScripts/config-files/config-lch.conf");
...
my $hso = WordNet::Similarity::hso->new($wn, "../PerScripts/config-files/config-hso.conf");
...
my $path = WordNet::Similarity::path->new($wn, "../PerScripts/config-files/config-path.conf");
...
my $random = WordNet::Similarity::random->new($wn, "../PerScripts/config-files/config-random.conf");
...
my $lesk = WordNet::Similarity::lesk->new($wn, "../PerScripts/config-files/config-lesk.conf");
...
my $vector = WordNet::Similarity::vector->new($wn, "../PerScripts/config-files/config-vector.conf");
...
my $vector_pairs = WordNet::Similarity::vector_pairs->new($wn, "../PerScripts/config-files/config-vector_pairs.conf");
...
sub get_similarity_scores
{
    ...
    if ($wps_p1[1] eq $wps_p2[1] && $wps_p1[1] ne "a")
    {
