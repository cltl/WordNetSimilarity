...
#these modules can compare adjectives
my @adj_comp_meas = ("lesk","vector","vector_pairs","random","hso");
...
# define output file
my $outfile = shift;
# measurement to use for calculation
my $measure = shift;
...
unless (defined $infile and defined $outfile and defined $measure) {
    ...
    print STDERR "Usage: similarity_scores_config_one_measure.pl inputfile outputfile measure\n";
    exit 1;
}
...
sub get_similarity_scores
{
    ...
    #all measurements should only apply to entries with identical pos
    if ($wps_p1[1] eq $wps_p2[1])
    {

        #only run if not adjective or measurement that can handle those
        if (($wps_p1[1] ne "a") || $adj_comp)
        {
